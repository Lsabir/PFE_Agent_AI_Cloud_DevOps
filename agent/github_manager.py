...
class GitHubManager:
    ...
    def ensure_repo_exists(self) -> str:
        ...
        if resp.status_code == 404:
            payload = {
                "name": self.repo,
                "description": "Infrastructure as Code - Agent IA DevOps",
                "private": False,
                "auto_init": True,
            }
    ...
    def _create_or_update_file(self, path: str, content: str, branch: str, commit_message: str) -> Dict[str, Any]:
    ...
    def push_workflow(self, branch: str, workflow_content: str) -> None:
        ...
        path = f".github/workflows/terraform-{workflow_name}.yml"
        commit_message = f"chore: add terraform workflow for {workflow_name}"
        self._create_or_update_file(path, workflow_content, branch, commit_message)
    ...
    def _list_workflow_runs(self, branch: str, event: Optional[str] = None) -> list[Dict[str, Any]]:
    ...
    def get_plan_output(self, run_id: int) -> str:
        ...
        lines = []
        for job in jobs:
            lines.append(f"Job {job['name']}: {job.get('conclusion', 'unknown')} ({job.get('status', 'unknown')})")
    ...
    def ensure_standard_pipeline(self) -> bool:
        """
        Pousse (ou met à jour) le pipeline Terraform standard dans le repo infra-provisioned.
        Toujours mis à jour pour garantir que la dernière version est active.
        Retourne True si créé/mis à jour, False si le contenu est identique.
        """
        ...
        if resp.status_code == 200:
            ...
            existing = _b64.b64decode(resp.json().get("content", "").replace("\n", "")).decode("utf-8")
            if existing == content:
                return False  # identique, rien à faire

        self._create_or_update_file(
            path=pipeline_path,
            content=content,
            branch=self._get_default_branch(),
            commit_message="chore: update standard Terraform pipeline (agent-managed)",
        )
        return True

    def get_repo_context(self) -> Dict[str, str]:
        """Lit tous les fichiers .tf et .tfvars existants à la racine du repo infra-provisioned."""
        default_branch = self._get_default_branch()
